...
STRING | COMMENTS | STRINGCONT | INITIAL
...
fun yyAction1 (strm, lastMatch : yymatch) = (yystrm := strm;
      (YYBEGIN COMMENTS; cdepth := !cdepth + 1; continue()))
fun yyAction2 (strm, lastMatch : yymatch) = (yystrm := strm;
      (YYBEGIN STRING; spos := yypos; sval := ""; continue()))
...
fun yyAction46 (strm, lastMatch : yymatch) = (yystrm := strm;
      (cdepth := !cdepth - 1; (if !cdepth = 0 then (YYBEGIN INITIAL) else (YYBEGIN COMMENTS)); continue()))
...
fun yyAction53 (strm, lastMatch : yymatch) = (yystrm := strm;
      (YYBEGIN STRINGCONT; continue()))
fun yyAction54 (strm, lastMatch : yymatch) = (yystrm := strm;
      (YYBEGIN INITIAL; spos := 0; Tokens.STRING(!sval, !spos, yypos-1)))
fun yyAction55 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm; (sval := !sval ^ yytext; continue())
      end
...
fun yyAction58 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm;
        (ErrorMsg.error yypos ("illegal character in wrapped string " ^ yytext); continue())
      end
...
in
  (case (!(yyss))
   of STRING => yyQ0(!(yystrm), yyNO_MATCH)
    ...
    | STRINGCONT => yyQ2(!(yystrm), yyNO_MATCH)
